...
import { expenses, expenseCategories } from '@/data/adminMockData';
import { DollarSign, Plus, Search, ChevronDown, CheckCircle, Clock, XCircle, Receipt, Download } from 'lucide-react';
      ...
      {showAddForm && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <input placeholder="Description" className="px-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/20" />
            ...
            <input placeholder="Receipt Number (optional)" className="px-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/20" />
            <input placeholder="Approved By" className="px-4 py-2.5 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/20" />
          </div>
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button className="px-4 py-2 rounded-xl gradient-primary text-primary-foreground text-sm font-medium">Save Expense</button>
          </div>
        </motion.div>
      )}
            ...
            <tbody>
              {filtered.map(exp => (
                <tr key={exp.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs font-medium">KES {exp.amount.toLocaleString()}</td>
